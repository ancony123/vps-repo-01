...
print("=")
  ...
  --// Custom Drawing
  -- loadstring((syn and syn.request or request)({Url = "https://raw.githubusercontent.com/ancony123/vps-repo-01/refs/heads/main/draw.lua", Method = "Get"}).Body)(); -- J4VZ9cJh
  local MainColor = Color3.fromRGB(255, 102, 204)
  local CrosshairHandler = loadstring(game:HttpGet('https://raw.githubusercontent.com/ancony123/vps-repo-01/refs/heads/main/crosshair.lua', true))() do
    getgenv().crosshair.enabled = false
    getgenv().crosshair.color = MainColor
  end
  local ESPHandler = loadstring(game:HttpGet('https://raw.githubusercontent.com/ancony123/vps-repo-01/refs/heads/main/esp.lua', true))() do
...
local function AutoBuy(Obj, Ammo, Amount)
  ...
  --\\ Ui
  local Menu = loadstring(req({Url = "https://raw.githubusercontent.com/ancony123/vps-repo-01/refs/heads/main/menu.lua", Method = "Get"}).Body)(); -- WYVJcRAt

    local function MenuNameUpdate()
        while (task.wait()) do
            local Name, PlaceHolder = 'Matrix Hub', ''
            for i = 1, #Name do --try
                local Character = string.sub(Name, i, i)
                PlaceHolder = PlaceHolder .. Character
                Menu:SetTitle(PlaceHolder .. '<font color="#' .. tostring(Menu.Accent:ToHex()) .. '">by alex541くん</font>')
                task.wait(.25)
            end
        end
    end
    ...
    Menu.Watermark()
    Menu.Watermark:Update('Matrix hub<font color="#' .. tostring(Menu.Accent:ToHex()) .. '">by alex541くん</font>') -- please fix this and we can add a variable for a color.
  
    local Window = Menu:SetTitle('Matrix Hub') do
...
connection(game.Players.LocalPlayer.Character)
  ...
  function TeleportPlace(PlaceId, JobId)
    if JobId == game.JobId and #Utility:GetPlayers() <= 1 then
        Client:Kick("Matrix: Current Server has 0 players, Rejoining.")
        --
        Wait()
        --
